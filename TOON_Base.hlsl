...
/* Description:
	Enables use of ENV reflection texture.
	Texture uses Image Sampler 1.
*/
#ifndef USE_ENV
#define USE_ENV 0
#endif
...
/* Description:
	Alternate version of ENV.
    Main color is added to ENV texture 
    instead of a multiply. 
*/
#ifndef USE_ENV_ADD
#define USE_ENV_ADD 0
#endif
...
#if USE_EYE && (USE_STAIN1 || USE_STAIN2 || USE_STAIN3 || USE_ENV || USE_ENV_ADD)
    #error "MUT and STAIN/ENV cannot be enabled at the same time!"
#endif
...
#if (USE_ENV || USE_ENV_ADD) && (USE_STAIN1 || USE_STAIN2 || USE_STAIN3 || USE_EYE)
    #error "ENV and STAIN/EYE cannot be enabled at the same time!"
#endif
...
// Exclude the use of the envReflectDirWS input when enabled
#if USE_ENV || USE_ENV_ADD
#define PS_TEXCOORD5_INPUT float4 envReflectDirWS : TEXCOORD5,
#else
#define PS_TEXCOORD5_INPUT
#endif
...
void main(
  ...
  PS_TEXCOORD3_INPUT	//contains lightDirOff for Hair shaders
  PS_TEXCOORD4_INPUT //contains worldPos for vfx lighting distance
  PS_TEXCOORD5_INPUT    //contains envReflectDirWS for ENV shaders
...
//-------------------------
	...
	//ENV texture UVs
#if USE_ENV || USE_ENV_ADD
		float2 envUV = normalize(envReflectDirWS.xyz).xy * 0.5 + 0.5;
#endif
...
//-------------------------
	// Linework Overlay
    float2 mainUV = UV0.xy;
	
#if USE_ENV || USE_ENV_ADD
		mainUV = envUV.xy;
#endif
	
    float4 overlay = Texture_LineworkOverlay.Sample(State_LineworkOverlay_s, mainUV);
    bool decalTex = true;
    float3 firstLayer;
    float3 secondLayer;
    float3 finalLayer = overlay.rgb;
    float thresholdStr = 0;
    ...
    if (decalTex)
        finalLayer.rgb = lerp(1.0, finalLayer.rgb, overlay.a);
    ...
    float3 finalClr = ENABLE_INK_SPEC ? fadedInk : lineColor;
	
#if USE_ENV
			finalClr = lerp(baseColor.rgb, finalClr, envReflectDirWS.www);
#endif
	
#else
	
		float3 finalClr = envReflectDirWS.www * finalLayer + baseColor.rgb;
	
#endif
	
    baseColor.rgb = finalClr;
